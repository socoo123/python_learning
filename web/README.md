...
| `bun run build:content` | 手动重新烘焙:`../` 源仓库 → `src/content/curriculum.json` |
| `bun run preview` | 预览生产构建 |

## 架构

- **内容烘焙**:`scripts/build-curriculum.ts`(Bun 原生跑 TS)扫描源仓库章节,产出
  `src/content/curriculum.json`(教程 / 作业骨架 / 测试源码 / mock 数据 / 闪卡)。
  **该文件提交进 git**,所以单独 clone `web/` 也能跑(可移植)。
- **运行方式**:
  - 🟢 Pyodide 章节(M1、M2、Ch23/25/26、M6):浏览器内 Monaco 编辑器 + ▶ 运行(Pyodide 跑 pytest)+ 终端红绿。
  - 🔒 Local 章节(M3、M5、Ch24/27):只读教程 + 本地运行命令(代码仍写本地 .py)。

## 内容更新

源课程文件改动后:`bun run build:content` 重新烘焙 → 提交 `src/content/curriculum.json`。
